"""
Tool para actualizar el campo "Correo usuario" en un lead de Kommo.
...
def update_data_Correo_Cliente(lead_id: int, correo: str) -> bool:
    """
    Actualiza el campo "Nombre del usuario" del lead.

    Args:
        lead_id: ID del lead.
        correo: Correo de contacto indicado por el cliente
    ...
    correo_custom_field = int(os.getenv("KOMMO_CORREO_FIELD_ID"))
    ...
    if response.status_code == 200:
        print(f"💳 Lead {lead_id}: Valor de correo del usuario actualizado a {correo}")
        return True
    else:
